...
\section{Conclusion}
surWe reviewed the pivot algorithms and pivot rules. Although there are certain pivot rules are designed to ensure algorithm’s finiteness, they are not utilized a lot in the softerware implementation in real life. We also presented some contemporary complexity analysis of pivot algorithms. Although there is no proved pivot algorithm has the polynomial complexity of general LO problems, for certain problems, such as the lattice polytopes, the complexity is proved to be better than worst case.
